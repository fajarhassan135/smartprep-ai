    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: bg, fontFamily: "'DM Sans', sans-serif", transition: "background 0.3s" }}>
        ...
        <div style={{ maxWidth: 600, margin: "0 auto", padding: "clamp(40px, 8vw, 64px) clamp(16px, 4vw, 24px)" }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: C.orange, marginBottom: 12 }}>AI Quiz</p>
          <h1 style={{ fontSize: "clamp(26px, 6.5vw, 36px)", fontWeight: 600, letterSpacing: "-0.03em", color: text, marginBottom: 8 }}>Set up your quiz</h1>
          <p style={{ fontSize: 14, color: sub, marginBottom: 48 }}>Choose your subject, exam level and mode to get started.</p>

          <div style={{ marginBottom: 24 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: text, display: "block", marginBottom: 10 }}>Subject</label>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {SUBJECTS.map((s) => (
                <button key={s} onClick={() => { setSubject(s); playTap(); }} style={{ flex: "1 1 30%", padding: "12px 8px", borderRadius: 12, border: subject === s ? `2px solid ${C.orange}` : `1px solid ${border}`, backgroundColor: subject === s ? "var(--accent-badge)" : bg, color: subject === s ? "var(--accent-ink)" : text, fontWeight: subject === s ? 500 : 400, fontSize: 13, cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s" }}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: text, display: "block", marginBottom: 10 }}>Difficulty</label>
            <div style={{ display: "flex", gap: 10 }}>
              {(["easy", "medium", "hard"] as Difficulty[]).map((d) => (
                <button key={d} onClick={() => { setDifficulty(d); playTap(); }} style={{ flex: 1, padding: "12px 8px", borderRadius: 12, border: difficulty === d ? `2px solid ${C.orange}` : `1px solid ${border}`, backgroundColor: difficulty === d ? "var(--accent-badge)" : bg, color: difficulty === d ? "var(--accent-ink)" : text, fontWeight: difficulty === d ? 500 : 400, fontSize: 13, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize", transition: "all 0.15s" }}>
                  {d}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: text, display: "block", marginBottom: 10 }}>Number of questions</label>
            <div style={{ display: "flex", gap: 10 }}>
              {[5, 10, 15, 20].map((n) => (
                <button key={n} onClick={() => { setQuestionCount(n); playTap(); }} style={{ flex: 1, padding: "12px 8px", borderRadius: 12, border: questionCount === n ? `2px solid ${C.orange}` : `1px solid ${border}`, backgroundColor: questionCount === n ? "var(--accent-badge)" : bg, color: questionCount === n ? "var(--accent-ink)" : text, fontWeight: questionCount === n ? 500 : 400, fontSize: 13, cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s" }}>
                  {n}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 40 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: text, display: "block", marginBottom: 10 }}>Mode</label>
            <div style={{ display: "flex", gap: 10 }}>
              {[
                { val: "practice", label: "Practice", desc: "Hints & explanations" },
                { val: "exam", label: "Exam", desc: "Timed, no hints" },
              ].map((m) => (
                <button key={m.val} onClick={() => { setExamMode(m.val as "practice" | "exam"); playTap(); }} style={{ flex: 1, padding: "14px", borderRadius: 12, border: examMode === m.val ? `2px solid ${C.orange}` : `1px solid ${border}`, backgroundColor: examMode === m.val ? "var(--accent-badge)" : bg, color: examMode === m.val ? "var(--accent-ink)" : text, fontWeight: examMode === m.val ? 500 : 400, fontSize: 13, cursor: "pointer", fontFamily: "inherit", textAlign: "left", transition: "all 0.15s" }}>
                  <div>{m.label}</div>
          ...
          <button onClick={generateQuiz} disabled={!subject || !board || loading} style={{ width: "100%", padding: "15px", borderRadius: 12, backgroundColor: !subject || !board ? C.garnet : C.orange, color: "#fff", fontWeight: 600, fontSize: 15, border: "none", cursor: !subject || !board ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: !subject || !board ? 0.5 : 1 }}>
    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: bg, fontFamily: "'DM Sans', sans-serif", transition: "background 0.3s" }}>
        <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px clamp(16px, 4vw, 40px)", borderBottom: `1px solid ${border}` }}>
          <a href="/dashboard" style={{ fontSize: 15, fontWeight: 600, color: text, textDecoration: "none", letterSpacing: "-0.03em" }}>
            <Logo />
          </a>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            {examMode === "exam" && (
              <div style={{ fontSize: 14, fontWeight: 600, color: timeLeft < 60 ? "var(--accent-ink)" : text }}>
                {formatTime(timeLeft)}
              </div>
            )}
        ...
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "clamp(32px, 7vw, 48px) clamp(16px, 4vw, 24px)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
            <span style={{ fontSize: 11, fontWeight: 600, padding: "4px 12px", borderRadius: 999, backgroundColor: bgMid, color: sub }}>
          ...
          <div style={{ background: "var(--card-strong)", border: `1px solid ${border}`, borderRadius: 18, padding: "28px 32px", marginBottom: 24, backdropFilter: "blur(16px)" }}>
            <p style={{ fontSize: 17, fontWeight: 600, color: text, lineHeight: 1.65, margin: 0 }}>{q.question}</p>
          </div>
          ...
          {q.type === "mcq" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
              {q.options?.map((opt, i) => {
                ...
                return (
                  <button key={i} onClick={() => { checkMCQ(letters[i]); }} disabled={answered}
                    className={`rise-in${isCorrect ? " is-correct" : ""}${isWrong ? " is-wrong" : ""}`}
                    style={{ ["--i" as string]: i, padding: "14px 18px", borderRadius: 12, border: isCorrect ? `2px solid #639922` : isWrong ? `2px solid #E24B4A` : isSelected ? `2px solid ${C.orange}` : `1px solid ${border}`, backgroundColor: isCorrect ? "rgba(99,153,34,0.1)" : isWrong ? "rgba(226,75,74,0.1)" : isSelected ? "var(--accent-badge)" : bg, color: isCorrect ? "#639922" : isWrong ? "#E24B4A" : text, fontSize: 14, cursor: answered ? "default" : "pointer", fontFamily: "inherit", textAlign: "left", display: "flex", alignItems: "center", gap: 12, transition: "all 0.15s" }}>
                    <span style={{ width: 28, height: 28, borderRadius: 999, backgroundColor: isCorrect ? "#639922" : isWrong ? "#E24B4A" : bgMid, color: isCorrect || isWrong ? "#fff" : sub, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600, flexShrink: 0 }}>
                      {letters[i]}
                    </span>
                    {opt.replace(/^[A-D]\)\s*/, "")}
                  </button>
          ...
          {q.type === "short" && (
            <div style={{ marginBottom: 24 }}>
              ...
              />
              {!answered && (
                <button onClick={checkShort} disabled={grading || !shortAnswer.trim()} style={{ marginTop: 12, padding: "12px 24px", borderRadius: 12, backgroundColor: C.orange, color: "#fff", fontWeight: 600, fontSize: 14, border: "none", cursor: "pointer", fontFamily: "inherit" }}>
          ...
          {answered && feedback[currentQ] && (
            <div style={{ padding: "16px 20px", borderRadius: 12, backgroundColor: feedback[currentQ] === "correct" ? "rgba(99,153,34,0.1)" : feedback[currentQ] === "incorrect" ? "rgba(226,75,74,0.1)" : "var(--accent-badge)", border: `1px solid ${feedback[currentQ] === "correct" ? "rgba(99,153,34,0.3)" : feedback[currentQ] === "incorrect" ? "rgba(226,75,74,0.3)" : "rgba(255,96,55,0.2)"}`, marginBottom: 24 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: feedback[currentQ] === "correct" ? "#639922" : feedback[currentQ] === "incorrect" ? "#E24B4A" : C.orange, marginBottom: 6 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              {hint ? (
                <div style={{ padding: "14px 18px", borderRadius: 12, backgroundColor: "var(--teal-badge)", border: "1px solid var(--teal-badge)", fontSize: 13, color: "var(--teal-ink)", lineHeight: 1.6 }}>
                  <strong style={{ fontWeight: 600 }}>Hint:</strong> {hint}
                </div>
              ) : (
                <button onClick={getHint} disabled={hintLoading} style={{ padding: "10px 20px", borderRadius: 10, border: `1px solid var(--teal-ink)`, backgroundColor: "transparent", color: "var(--teal-ink)", fontSize: 13, fontWeight: 600, cursor: hintLoading ? "default" : "pointer", fontFamily: "inherit" }}>
          ...
          {!answered && (
            <button onClick={skipQuestion} style={{ width: "100%", padding: "13px", borderRadius: 12, backgroundColor: "transparent", color: sub, fontWeight: 600, fontSize: 14, border: `1px solid ${border}`, cursor: "pointer", fontFamily: "inherit" }}>
              {currentQ + 1 >= questions.length ? "Skip & see results" : "Skip this question"}
            </button>
          )}

          {answered && (
            <button onClick={nextQuestion} style={{ width: "100%", padding: "14px", borderRadius: 12, backgroundColor: C.orange, color: "#fff", fontWeight: 600, fontSize: 15, border: "none", cursor: "pointer", fontFamily: "inherit" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: bg, fontFamily: "'DM Sans', sans-serif", transition: "background 0.3s" }}>
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "clamp(40px, 8vw, 64px) clamp(16px, 4vw, 24px)", textAlign: "center" }}>
        <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: C.orange, marginBottom: 12 }}>Quiz complete</p>
        <h1 style={{ fontSize: "clamp(34px, 9vw, 56px)", fontWeight: 600, letterSpacing: "-0.03em", color: text, marginBottom: 8 }}>{percentage}%</h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 16, marginBottom: 48 }}>
          {[
            ...
            <div key={s.label} style={{ backgroundColor: bgMid, borderRadius: 16, padding: "20px", border: `1px solid ${border}` }}>
              <div style={{ fontSize: "clamp(22px, 5vw, 28px)", fontWeight: 600, color: s.color }}>{s.value}</div>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
          <button onClick={() => { finishedRef.current = false; setHint(""); setMode("setup"); setScore(0); setCurrentQ(0); setAnswers([]); setFeedback([]); setVerdicts([]); setAnswered(false); setSelectedOption(""); setShortAnswer(""); }} style={{ padding: "13px 28px", borderRadius: 12, backgroundColor: C.orange, color: "#fff", fontWeight: 600, fontSize: 14, border: "none", cursor: "pointer", fontFamily: "inherit" }}>
            Try again
          </button>
          <a href="/dashboard" style={{ padding: "13px 28px", borderRadius: 12, backgroundColor: bgMid, color: text, fontWeight: 600, fontSize: 14, border: `1px solid ${border}`, textDecoration: "none", display: "inline-flex", alignItems: "center" }}>
            Dashboard
          </a>
          <a href="/history" style={{ padding: "13px 28px", borderRadius: 12, backgroundColor: bgMid, color: text, fontWeight: 600, fontSize: 14, border: `1px solid ${border}`, textDecoration: "none", display: "inline-flex", alignItems: "center" }}>
            View history →
          </a>
        </div>
      </div>
