  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: bg, fontFamily: "'DM Sans', sans-serif", transition: "background 0.3s" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "48px clamp(16px, 4vw, 40px)" }}>
        <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: C.orange, marginBottom: 12 }}>Leaderboard</p>
        <h1 style={{ fontSize: "clamp(26px, 6.5vw, 36px)", fontWeight: 600, letterSpacing: "-0.03em", color: text, marginBottom: 8 }}>Class rankings</h1>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "60px", background: "var(--card-strong)", borderRadius: 20, border: `1px solid ${border}` }}>
            <div style={{ fontSize: 16, fontWeight: 600, color: text, marginBottom: 8 }}>No rankings yet</div>
                ...
                {top3.map((row, i) => {
                  ...
                  return (
                    <div key={row.user_id} style={{ background: rank === 1 ? `linear-gradient(135deg, ${C.orange}, ${C.orangeDark})` : "var(--card-strong)", border: `1px solid ${rank === 1 ? C.orange : border}`, borderRadius: 20, padding: "28px 20px", textAlign: "center", backdropFilter: "blur(16px)" }}>
                      <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: rank === 1 ? "rgba(255,255,255,0.85)" : sub, marginBottom: 12 }}>Rank {rank}</div>
                      <div style={{ fontSize: 15, fontWeight: 600, color: rank === 1 ? "#fff" : text, marginBottom: 4 }}>
                      ...
                      <div style={{ fontSize: "clamp(22px, 5vw, 28px)", fontWeight: 600, color: rank === 1 ? "#fff" : C.orange }}>{row.avg_score}%</div>
                ...
                return (
                  <div key={row.user_id} style={{ background: isYou ? "var(--accent-badge)" : "var(--card-strong)", border: isYou ? `1px solid ${C.orange}` : `1px solid ${border}`, borderRadius: 14, padding: "16px clamp(14px, 4vw, 24px)", display: "flex", alignItems: "center", gap: 20, backdropFilter: "blur(16px)" }} className="row-stack">
                    ...
                    <div className="row-stack-tail" style={{ display: "flex", gap: 24, alignItems: "center" }}>
                      ...
                      <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: text }}>{row.best_score}%</div>
                        <div style={{ fontSize: 10, color: sub }}>best</div>
                      </div>
                      <div style={{ fontSize: 18, fontWeight: 600, color: C.orange, minWidth: 48, textAlign: "right" }}>
                        {row.avg_score}%
                      </div>
                    </div>
                  </div>
